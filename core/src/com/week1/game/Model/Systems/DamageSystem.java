 ...
 */
public class DamageSystem implements ISystem, Subscriber<DamageEvent>, Publisher<DeathSystem.DeathEvent> {

    private ArrayList<Subscriber<DeathSystem.DeathEvent>> deathSubscribers = new ArrayList<>();
    ...
    public void update(float delta) {
        for (DamageEvent damageEvent: damageEvents) {
            ...
            Gdx.app.log("DamageSystem", "Dealing Damage to " + damageEvent.victimID);
            ...
            if (victimHealth.curHealth <= 0) {
                ...
                for (Subscriber<DeathSystem.DeathEvent> deathEventSubscriber: deathSubscribers) {
                    deathEventSubscriber.process(new DeathSystem.DeathEvent(damageEvent.victimID, damageEvent.damagerID));
                }
            }
        }
        damageEvents.clear();
    ...
    @Override
    public void addSubscriber(Subscriber<DeathSystem.DeathEvent> subscriber) {
